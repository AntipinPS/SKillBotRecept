...
from aiogram.filters import Command, CommandObject; from aiogram.types import Message; from aiogram.utils.keyboard import ReplyKeyboardBuilder
...
async def category_search_random(message: Message, command: CommandObject, state: FSMContext):
    if command.args is None:
        await message.answer("Ошибка: Вы не указали число")
        return
...
async def recipes_by_category(message: types.Message, state: FSMContext):
    ...
    response = as_list(
        as_marked_section("Есть такие рецепты: ", *random_meals_in_russian),
    )
    ...
    async with aiohttp.ClientSession() as session:
        for id in random_recipes_id['random_recipes']:
            async with session.get(f'http://www.themealdb.com/api/json/v1/1/lookup.php?i={id}', ) as resp:
                ...
                translator = Translator()
                recipe_en = [data['meals'][0]['strMeal'],
                             data['meals'][0]['strInstructions'],
                             ', '.join(ingredients)]
                for rec in recipe_en:
                    recipe_ru = translator.translate(rec, dest="ru")
                    recipe = f"{recipe_ru.text} \n\n" \
                             f"Рецепт:\n " \
                             f"{recipe_ru.text} \n\n" \
                             f"Ингредиенты: {recipe_ru.text}"
